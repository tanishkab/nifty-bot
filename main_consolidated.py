...
#     """
#     Real option LTP Angel API se fetch karo - NO ESTIMATION!
...
#
#         # Try ATM and nearby strikes for better success
#         for strike_delta in [0, sg, -sg, 2*sg, -2*sg, 3*sg, -3*sg]:
#             try_strike = strike + strike_delta
#             token, opt_sym = get_option_token(sym, try_strike, opt_type)
#
...
#                     logger.debug(f"LTP fetch attempt failed for {opt_sym}: {e1}")
#                     continue
#
#         # If all attempts fail, log critical warning
#         logger.error(f"❌ CRITICAL: Could not fetch REAL LTP for {sym} {opt_type} @ {spot}")
#         logger.error(f"❌ Token count: {len(ST.opt_tokens)}, API connected: {ST.api is not None}")
#
#         # Fallback - but log it prominently
#         est = round(spot * 0.005, 2)
#         logger.warning(f"⚠️  USING ESTIMATE: Rs.{est} (0.5% of spot)")
#         return est
#
#     except Exception as e:
#         logger.error(f"❌ Option LTP error: {e}")
#         est = round(spot * 0.005, 2)
#         logger.warning(f"⚠️  FALLBACK ESTIMATE: Rs.{est}")
#         return est
#
# # ═══════════════════════════════════════════════════════
...
#         # EXIT check
#         if pos:
#             # REAL-TIME PREMIUM from Angel API
...
#                     f"Strike:{strike} LTP:{opt_price:.2f} (REAL) | "
...
#     if not ST.running:  # Only update if not running
#         lbl_sym.config(text=v_sym.get())
#
# v_sym.trace_add("write", on_symbol_change)
...
#     if not ST.running:  # Only update if not running
#         lbl_mode.config(text=v_mode.get().upper())
#         # Change color based on mode
#         if v_mode.get() == "live":
#             lbl_mode.config(fg=RED)
#         elif v_mode.get() == "backtest":
#             lbl_mode.config(fg=GOLD)
#         else:
#             lbl_mode.config(fg=ACC)
#
# v_mode.trace_add("write", on_mode_change)
...
#     lbl_sym.config(text=CFG["SYMBOL"])
#     lbl_mode.config(text=ST.mode.upper())
#     lbl_run.config(text="RUNNING", fg=GRN)
#
